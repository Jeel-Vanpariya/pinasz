<template>
  <Panel header="Container Types" class="p-4">
    <template #icons>
    ...
    <DataTable :value="data" v-model:filters="filters" :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]" dataKey="id" removableSort paginator>
      ...
      <Column field="type_name" header="Type Name" sortable>
  ...
  <Dialog v-model:visible="visible" header="Add Container Type" modal :draggable="false" :style="{ width: '30vw' }">
    <Form class="mt-4 container__type__add" :validation-schema="schema" :initialValues="initialValue" @submit="onSubmit">
      <Field name="id" v-slot="{ value, handleChange }">
        <InputText type="hidden" :model-value="value" @update:model-value="handleChange" />
      </Field>
      <Field name="type_name" v-slot="{ value, errorMessage, handleChange }">
        <span class="p-float-label">
          <InputText id="type_name" type="text" :model-value="value" :class="{ 'p-invalid': errorMessage }" @update:model-value="handleChange" />
...
const onSubmit = async (data: any, { resetForm }: any) => {
  const res = await store.dispatch('saveContainerType', data);
  if (res.status == 'success') {
...
const confirmDelete = (id: number) => {
  confirm.require({
    ...
    accept: async () => {
      store.state.spinner = true;
      const res = await store.dispatch('deleteContainerType', { id: id });
      store.state.spinner = false;
      if (res.status == 'success') {
        getContainerType();
      }
